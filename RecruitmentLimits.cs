...
{
    public class RecruitmentMod : MBSubModuleBase
    {
        // 引用 SoldierLimitBehavior 中的 troopLimits 和 troopLimitValues
        ...
        {
            var harmony = new Harmony("com.example.recruitmentpatch");
            harmony.Patch(
                original: AccessTools.Method(typeof(MobileParty), "RecruitVolunteersFromNotable"),
                prefix: new HarmonyMethod(typeof(RecruitmentMod), nameof(Prefix_RecruitVolunteersFromNotable))
            );
        }

        public static bool Prefix_RecruitVolunteersFromNotable(MobileParty mobileParty, Settlement settlement)
        {
            // 检查队伍成员数量是否超过上限
        ...
        private static void GetRecruitVolunteerFromIndividual(MobileParty mobileParty, CharacterObject character, Hero hero, int troopIndex)
        {
            // 这里是招募志愿者的具体逻辑
            // 你可以根据需要实现或调用原游戏的方法
        ...
        private static int CalculateTotalSoldiers(List<string> troopIds, MobileParty mobileParty, Settlement settlement)
        {
            int total = 0;
